...
class MyCompleteQuest extends StatefulWidget {
  const MyCompleteQuest({
    Key? key,
  }) : super(key: key);

  @override
  _MyCompleteQuestState createState() => _MyCompleteQuestState();
}

class _MyCompleteQuestState extends State<MyCompleteQuest> {
  final MyCompleteQuestController _questController =
      Get.find<MyCompleteQuestController>();
  ...
  Widget _buildList(int index, Quest attr) {
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () async {
          final result = await Get.to(() => DetailQuestPage(quest: attr, eBinding: eQuestBinding.completed,),
